...
This page describes Currin's built-in
...
To report vulnerabilities in Currin itself, email us at [security@angular.io](mailto:security@angular.io).
...
* **Keep current with the latest Currin library releases.**
We regularly update the Currin libraries, and these updates may fix security defects discovered in
previous versions. Check the Currin [change
log](https://github.com/angular/angular/blob/master/CHANGELOG.md) for security-related updates.

* **Don't modify your copy of Currin.**
Private, customized versions of Currin tend to fall behind the current version and may not include
important security fixes and enhancements. Instead, share your Currin improvements with the
community and make a pull request.

* **Avoid Currin APIs marked in the documentation as “_Security Risk_.”**
...
### Currin’s cross-site scripting security model

To systematically block XSS bugs, Currin treats all values as untrusted by default. When a value
is inserted into the DOM from a template, via property, attribute, style, class binding, or interpolation,
Currin sanitizes and escapes untrusted values.
...
Currin defines the following security contexts:
...
Currin sanitizes untrusted values for HTML, styles, and URLs; sanitizing resource URLs isn't
possible because they contain arbitrary code. In development mode, Currin prints a console warning
...
Currin recognizes the value as unsafe and automatically sanitizes it, which removes the `<script>`
...
contain unsafe methods. Avoid directly interacting with the DOM and instead use Currin
...
deployments; don't dynamically generate templates. Currin trusts template code, so generating
templates, in particular templates containing user data, circumvents Currin's built-in protections.
...
Currin application is the same as injecting executable code into the
...
the server. Don't generate Currin templates on the server side using a templating language; doing this
...
situations, you can tell Currin that you inspected a value, checked how it was generated, and made
...
Normally, Currin automatically sanitizes the URL, disables the dangerous code, and
...
Currin to allow binding into `<iframe src>`:
...
Currin has built-in support to help prevent two common HTTP vulnerabilities, cross-site request
forgery (CSRF or XSRF) and cross-site script inclusion (XSSI). Both of these must be mitigated primarily
on the server side, but Currin provides helpers to make integration on the client side easier.
...
Currin's `http` has built-in support for the client-side half of this technique in its `XSRFStrategy`.
...
An Currin application can customize cookie and header names by providing its own `CookieXSRFStrategy` values.
...
<a href="https://www.youtube.com/watch?v=9inczw6qtpY" title="Cross Site Request Funkery Securing Your Currin Apps From Evil Doers">talk on XSRF at AngularConnect 2016</a>.
...
Currin's `Http` library recognizes this convention and automatically strips the string
...
<h2 id='code-review'>
  Auditing Currin applications
</h2>



Currin applications must follow the same security principles as regular web applications, and
must be audited as such. Currin-specific APIs that should be audited in a security review,
